...
function payLoadIsExpectedChatMessage(
  ...
  expect(msg.displayName, Failure.incorrectDisplayName).toBe(expectedAuthorDisplayName);
...
describe('\nAutomated Chat Room REST Tests: PostAChatMessage\n', () => {
  it('should be able to post a chat message from self', async () => {
    ...
    payLoadIsExpectedChatMessage(res.data.payload, Data.msg1ByUser1Saved, Data.user1.extra);
  });

  it('should not be able to post a chat message with no text', async () => {
    checkResponse({
      actual: await axios.post(app.url + '/chat/messages', Data.msg1ByUser1WithNoText, {
        headers: { Authorization: `Bearer ${tokenForUser1}` },
        validateStatus: () => true
      }),
      expected: {
  ...
  it('should not be able to post a chat message with no author', async () => {
    checkResponse({
      actual: await axios.post(app.url + '/chat/messages', Data.msg1ByUser1WithNoAuthor, {
        headers: { Authorization: `Bearer ${tokenForUser1}` },
        validateStatus: () => true
      }),
      expected: {
  ...
  it('should not be able to post a chat message for another user (a user cannot impersonate another user when posting a chat message)', async () => {
    ...
    checkResponse({
      actual: await axios.post(app.url + '/chat/messages', Data.msg1ByUser1ImpersonatingUser2, {
        headers: { Authorization: `Bearer ${tokenForUser1}` },
        validateStatus: () => true
      }),
      expected: {
  ...
  it('should not be able to post a chat message for a nonexisting user', async () => {
    checkResponse({
      actual: await axios.post(app.url + '/chat/messages', Data.msg1ByUser1WithNonExistingAuthor, {
        headers: { Authorization: `Bearer ${tokenForUser1}` },
        validateStatus: () => true
      }),
      expected: {
...
describe('\nAutomated Chat Room REST Tests: DisplaysChatMessages\n', () => {
  it('will retrieve an empty collection of chat messages when no chat message exists', async () => {
    await app.db.init();
    const tokenForUser1Again = await getTokenAndCheck(app.url, Data.user1, Data.savedUser1);
    const res = checkResponse({
  ...
  it('should be able to retrieve all chat messages when chat messages exist', async () => {
    ...
    expect(actualMsgs, Failure.missingChatMessage).toContainEqual(Data.msg1ByUser1);
    expect(actualMsgs, Failure.missingChatMessage).toContainEqual(Data.msg2ByUser2);
  });
